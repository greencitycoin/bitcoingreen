...
- [Development guidelines](#development-guidelines)
    - [General Green City Core](#general-greencity-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play GCCs (tGCC)" on the test network, if you
...
Green City Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/greencity/greencity/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_greencity
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_greencity --log_level=test_suite
$ valgrind -v --leak-check=full src/greencityd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_greencity.coverage/index.html`.
```

**Sanitizers**

Green City Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Green City Core, where everyone uses
...
pay attention to for reviewers of Green City Core code.

General Green City Core
--------------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Green City Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Green City Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Green City Core so that they can be integrated
...
file descriptors it will cause problems with Green City's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Green City's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:greencity/greencity.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `greencity-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `greencity-cli` and the GUI debug console use this table to determine how to
