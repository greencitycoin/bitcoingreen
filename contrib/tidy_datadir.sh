...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Green City database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Green City datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Green City datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Green City 0.7 datadir."
    ;;
  3)
    echo "Detected Green City pre-0.8 datadir."
    ;;
  4)
    echo "Detected Green City 0.8 datadir."
    ;;
esac
